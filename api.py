...
def api_services():
    ...
    with bd.creer_connexion() as conn:
        services = bd.get_service_all(conn)
        app.logger.info(f"Affichage des services dans la page d'accueil par l'utilisateur (ID= {session.get('id_utilisateur')})")


    return jsonify(code=200, services=services)

@bp_api.route("/reservations/disponibilite/<int:id_service>", methods=["GET"])
def api_disponibilite_reservation(id_service: int):
    ...
    try:
        ...
        app.logger.info(
    f"Verification dispo: service={id_service}, date={date_obj} ({type(date_obj)}), heure={heure_obj} ({type(heure_obj)})"
)
    except ValueError:
